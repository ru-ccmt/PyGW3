...
subroutine Ewald_Summation(sgm, pq, lam_max, pos, br2, rbas, alat, Vol, ortho, r_cf, g_cf, eta,  ndf)
  ...
  ! The result is sgm(l,m,dR_ij) = \sum_{R} gamma(l)*e^(-i*q*(R+dR_{ij})) * Y_lm(\hat{R_dR_{ij}})
  implicit none
  ...
  do idf=1,ndf
     do jdf=idf,ndf
        ...
        do i1=1,np
           rp_aa(1:3) = rstr(1:3, indx(i1))  ! real space lattice R_aa, which is R_aa = R+dR_{ij}
           rleng      = rstr(4,   indx(i1))  ! length of R_aa
           
           !! calculate the values of the spherical harmonics at rp_aa
           call ylm(ylam, rp_aa, lam_max)                 ! Y_lm(\hat{R_aa})
           q_x_aa   = dot_product(rp_aa, mq)              ! R_aa*(-q)
           exp_q_aa = cmplx (cos(q_x_aa), sin(q_x_aa), 8) ! e^(-i*q*R_aa)
           gausr   = exp(-rleng**2/eta**2)                ! exp(-|R_aa|^2/eta^2)
           gammaor = sqrt(pi)*erfc(rleng/eta)/rleng       ! sqrt(pi)*erfc(|R_aa|/eta)/|R_aa| == gamma(l=0) in notes
           
           !write(6,'(A,I4,F12.5,5x,3F12.5,5x,2F12.5)') 'cc', i1, rleng, rp_aa(:), gausr, gammaor
           
           cm(1) = gammaor * exp_q_aa * ylam(1)  ! gamma(l=0) * e^(-i*q*R_aa) * Y_{lm=0}(\hat{R_aa})
           do l = 1,lam_max
              gammaor = gammaor * (l-0.5d0)/rleng + rleng**(l-2) * gausr / ( eta**(2*l-1) )  ! gamma(l) can be computed by recursion (see notes)
              ttmp = gammaor * exp_q_aa        ! gamma(l)*e^(-i*q*R_aa)
              do m = -l,l
                 ilm = l*l + l + m + 1
                 cm(ilm) =  ttmp * ylam(ilm)   ! cm(ilm) = gamma(l)*e^(-i*q*R_aa) * Y_lm(\hat{R_aa})
              enddo ! m
           enddo ! l
           stmp1(:) = stmp1(:) + cm(:) ! Here we have stmp1 = \sum_{R} gamma(l)*e^(-i*q*R_aa) * Y_lm(\hat{R_aa}), where R_aa=R+dR_{ij}
        ...
        do i1=1,ng
           ...
           exp_g_aa = cmplx( cos(g_x_aa), sin(g_x_aa), 8)  ! e^(i*G*r_aa)
           !
...
subroutine MT_Coulomb(vmat, vq, iat, loctmatsize, big_l, nmix, im_start, tilg, rrint, djmm, rtlij, sgm, mult, nat, ndf, max_nmix, dimdj, nijrm, tsize, lam_max)
  ...
  im = im_start(iat)
  do ieq = 1, mult(iat)        !* Loop over equivalent atoms corresponding to iat
     idf = idf+1
     ijdf=(idf*(idf+1))/2      !* ijdf for jdf==idf
     do irm = 1, nmix(iat)     !* Loop over mixed (product basis) functions:
        l1 = big_l(irm,iat)    !* l1 is orbital momentum of product basis function
        do m1 = -l1,l1         !* m1 of product basis function
           im = im + 1         !* combined index (iatom,l1,m1)
           jdf=0               !* index of anothe atom
           do jat=1,nat          !* over inequivalent only
              jm = im_start(jat) !* 
              do jeq=1,mult(jat) !* over those atoms that are equivalent to jat
                 jdf=jdf+1       !* like idf but for the second atom
                 iidf = min(idf,jdf) !* smaller of the two atoms
                 jjdf = max(idf,jdf) !* larger of the two atoms
                       ...
                       if (.not. (q_is_zero .and. l1.eq.0 .and. l2.eq.0) ) then
                          ...
                          do m3 = -l1,l1    ! -m3 is m1 if atom1's coordinate axis coincides with the global axis. In general, there is extra rotation.
                             do m4 = -l2,l2 !  m4 is m2 if atom2's coordinate axis coincides with the global axis.
                                ! sgm is Ewald sum : sgm(l,m,R_ij) == \sum_R Y_{l,m}(R+R_ij) e^{-iq*(R+R_ij)}/|R+R_ij|^{l+1}
                                if ( idf <= jdf) then ! because Ewald sum is available only for R_i<R_j
                                   lm34 = (l1+l2+1)*(l1+l2) + m4 + m3 + 1  ! Index for the Ewald sumation, l=l1+l2, m=m3+m4
                                   stc = sgm(lm34, ijdf)       ! Ewald summation \sum_R Y_{l1+l2,m3+m4} e^{-iq*R}/R^{l1+l2+1}
                                   ...
                                   ! It turns out that Evald sumation has the property (check for yourself) sgm(l,m,-R) = (-1)^{l+m} * conj(sgm(l,-m,R)). This is because Y_lm(-R)=(-1)^l*Y_{l,m}(R) and conj(Y_lm(-R))=(-1)^{l+m}Y_{l,-m}(R)
                                   lm34 = (l1+l2+1)*(l1+l2) -m4 - m3 + 1  ! Here we need conjugated Ewald sum of e^{-iq*(-R)}/R^{l1+l2+1} Y_{l1+l2,m3+m4}(-R) = [e^{-iq*R}/R^{l1+l2+1} Y_{l1+l2,-m3-m4} ]^*(-1)^{m3+m4+l1+l2}
                                   stc = conjg( sgm(lm34, ijdf) )         ! need conjugated Ewald sum conj(stc(l1+l2,-m3-m4)) 
                                   minu = 1-2*mod(abs(m4+l1+l2),2)        ! == (-1)**(m4+l1+l2) : note that we have extra (-1)^(m3) and from sgm(-R) we get (-1)^{l1+l2+m3+m4}, hence we are left with (-1)^{l1+l2+m4}
                                   !if (minu .ne. minu1) print *, 'ERROR', m4+l1+l2, minu, minu1
                                   !write(6,'(I4,1x,I4,1x,2F16.8)') ijdf, lm34, stc
                                endif
                                tg = gettildeg(l1,l2,m3,m4,tilg,tsize)
                                ! minu*stc*tg = \sum_R (-1)^(m3) * Y_{l1+l2,m3+m4}(R+R_ij) e^{-iq*(R+R_ij)}/|R+R_ij|^{l1+l2+1} * tilde(l1,l2,m3,m4) == <l1,-m3,r| e^{-i*q*(r-r')}/|r-r'| |l2,m4,r'>
                                !@@??
                                ...
                                !if (im.eq.1 .and. (jm.eq.212 .or. jm.eq.216 .or. jm.eq.219 .or. jm.eq.223 .or. jm.eq.226 .or. jm.eq.230 .or. jm.eq.233 .or. jm.eq.237)) then
                                !   write(6,'(A,I3,A,I3,A,I3,A,I3,A,I3,A,I3,A,I3,A,I3,A,2F15.10,A,2F15.10,A,2F15.10,A,F15.10,A,2F15.10)') 'l1=',l1,',m1=',m1,',l2=',l2,',m2=',m2,',m3=',m3,',m4=',m4,',idf=',idf,',jdf=',jdf,',sgm=',(stc*minu),',dj3=',dj3,',dj4=',dj4,',tg=',tg,',sum=',sum34
                                !endif
                             enddo ! m4
                          enddo ! m3
                          ! rtlij = < r^L_{im} | u_{im,iat} > * <r^L_{jm}| u_{jm,jat}>
                          ! Vmat = rtlij * Ewald_sum(l1+l2,-m1+m2) * (-1)^m1 * tildeg(l1,l2,-m1,m2). We also take into account the rotation when global and local axis are not alligned.
                          Vmat(im,jm) = rtlij(jrm,irm,jat,iat) * sum34
                          !write(6,'(A,I3,A,I3,A,2F15.10)') 'Vmat(',im,',',jm,')=',Vmat(im,jm)  !@@??
...
subroutine pw_get_short_and_phase(glen0, phase, ngs, gind4, gind, glen, pos, mult, nk, nat, ndf)
  ...
  real(8)    :: dpos(3), G_times_dR
  complex*16 :: dphase(ngs)
        ...
        do jat=1,nat
           do jeq=1,mult(jat)
              ...
              if (idf .eq. jdf) then
                 ...
                 dphase(:) = 0.0
                 do ipw = 2, nk                 ! over all G's, excepy G=0
                    G_times_dR = dot_product( gind(ipw,:), dpos)
                    expg = exp( -G_times_dR * two_pi_i )  ! exp(-(G*(R_i-R_j)*2*pi*1j)
                    igl = gind4(ipw)                      ! igl is index in the shorter array, which distinguishess only length of G
                    dphase(igl) = dphase(igl) + expg      ! phase for the entire set of G vectors which have equal length!
                    !phase(idf,jdf,igl) = phase(idf,jdf,igl) + expg      ! phase for the entire set of G vectors which have equal length!
                    !dp = dot_product( gind(ipw,:), dpos)     !@@??
                    !write(6,'(3I5,1x,I7,2x,2F10.4,2x,2F10.4,4x,2F10.4)') idf, jdf, igl, ipw, expg, dp, phase(idf,jdf,igl)   !@@??
                 enddo ! ipw
                 phase(idf,jdf,:) = dphase(:)
              endif
              !write(6,*) 'finally phase' !@@??
              !do igl=1,ngs
