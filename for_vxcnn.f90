...
subroutine Compute_M_P_M_notetra2(eps, epsw1, kcw, minm2, enk, enq, dwo, dwe, omega, pm, iq, Noactual, Neactual, coef, ankp, matsize, NoNe, No, Ne, nom,No_,Ne_,NoNe_)
  IMPLICIT NONE
  COMPLEX*16, intent(inout) :: eps(matsize,matsize,nom)
  COMPLEX*16, intent(inout) :: epsw1(matsize,nom)
  COMPLEX*16, intent(in) :: minm2(NoNe,matsize)
  REAL*8, intent(in)  :: enk(No), dwo(No)  ! enk(ncg+ks.ibmin_tetra+1), weight-like f(E_{ik,io})
  REAL*8, intent(in)  :: enq(Ne), dwe(Ne)  ! enq(ks.nbmaxpol-ncmb), weight-like f(-E_{jk,ie})
  REAL*8, intent(in)  :: omega(nom)
  REAL*8, intent(in)  :: kcw(No_,Ne_,nom) ! No=20,Ne=205,nom=32,Noactial=20,Ne=205
  COMPLEX*16, intent(in) :: pm(NoNe_)
  INTEGER, intent(in) :: Noactual, Neactual, iq, ankp
  ...
  do iw=1,nom
     ...
     do io=1,Noactual ! ncg+ks.ibmin_tetra+1
        do ie=1,Ne    ! ks.nbmaxpol-ncbm
           ioie = (io-1)*Ne + ie
           dE = enq(ie) - enk(io)
           kcw2 = -(2.0*dwe(ie)*dwo(io)/ankp)*dE/( omega(iw)*omega(iw) + dE*dE)
